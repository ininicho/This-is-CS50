...
void print_winner(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = 0; j < (candidate_count - 1); j++)
        {
            ...
            {
                string dummyn = candidates[j + 1].name;
                int dummys = candidates[j + 1].votes;
                candidates[j + 1].name = candidates[j].name;
                candidates[j + 1].votes = candidates[j].votes;
                candidates[j].name = dummyn;
                candidates[j].votes = dummys;
            }
        }
    }

    for (int x = candidate_count - 1; x >= 0; x = x - 1)
    {
